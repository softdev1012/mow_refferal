...
import { IOwnerItemProps } from "../../types/owner";
import { CustomAvatar } from "..";

const today: string = format(new Date(), "yyyy-MM-dd");

const OwnerItem: React.FC<IOwnerItemProps> = ({ owner }) => {
    ...
    <TableRow className="text-center bg-white border-b dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="flex items-center justify-center">
          <div
            className={`h-2.5 w-2.5 rounded-full me-2 ${
              owner.clanStatus ? "bg-green-500" : "bg-red-500"
            }`}
          ></div>
          {owner.clanStatus ? "Active" : "Inactive"}
        </div>
      </TableCell>
      <TableCell
        align="center"
      ...
      >
        ...
        >
          <div
            className={`h-2.5 w-2.5 rounded-full me-2 ${
              owner.profileStatus ? "bg-green-500" : "bg-red-500"
            }`}                                                                                                                    
          ></div>
          <span>{owner.profileStatus ? "Active" : "Inactive"}</span>
        </div>
      </TableCell>
